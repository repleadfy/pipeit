...
async function login() {
    ...
    const code = await new Promise((resolveP, rejectP) => {
        const server = http.createServer((req, res) => {
            ...
            server.close();
            if (err)
                return rejectP(new Error(`auth error: ${err}`));
            if (!c)
                return rejectP(new Error("no code returned"));
            if (s !== state)
                return rejectP(new Error("state mismatch"));
            resolveP(c);
        });
        server.listen(port, () => {
    ...
    if (!tokR.ok)
        throw new Error(`token exchange failed: ${tokR.status} ${await tokR.text()}`);
...
async function getToken(forceLogin = false) {
    if (!forceLogin && existsSync(TOKEN_PATH))
...
async function upload(filePath, opts) {
    ...
    const post = (token) => fetch(`${BASE}/api/docs`, {
        method: "POST",
...
function parseArgs(argv) {
    const out = { file: undefined, isPublic: false, forceNew: false, logout: false, help: false };
    for (const a of argv) {
        if (a === "--public")
            out.isPublic = true;
        else if (a === "--new")
...
function printHelp() {
    process.stderr.write(`pipeit-upload — upload a markdown file to pipeit.live\n\n` +
        `Usage:\n` +
